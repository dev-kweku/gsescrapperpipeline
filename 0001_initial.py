...
def upgrade() -> None:
    ...
    op.create_table("daily_prices",
        ...
        sa.Column("share_code",   sa.String(20), sa.ForeignKey("gse.stock_listings.share_code"), nullable=False),
    ...
    op.create_table("scrape_jobs",
        sa.Column("id",       sa.BigInteger, primary_key=True, autoincrement=True),
        sa.Column("job_id",   postgresql.UUID(as_uuid=True), nullable=False, unique=True, server_default=sa.text("uuid_generate_v4()")),
        sa.Column("job_type", sa.String(50), nullable=False, server_default="daily"),
        sa.Column("status",   sa.Enum("pending","running","completed","failed","skipped", name="scrapestatus", schema="gse"), nullable=False, server_default="pending"),
    ...
    op.create_table("scrape_errors",
        sa.Column("id",         sa.BigInteger, primary_key=True, autoincrement=True),
        sa.Column("job_id",     postgresql.UUID(as_uuid=True), sa.ForeignKey("gse.scrape_jobs.job_id", ondelete="CASCADE")),
...
def downgrade() -> None:
    ...
    op.execute("DROP TYPE IF EXISTS gse.scrapestatus")
